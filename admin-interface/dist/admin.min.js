...
angular.module('yetienceApp')
    .controller('ListController', ['$scope', 'SettingsService', '$state', '$rootScope', 'UtilsService', 'CommService', 'UpgradeService', '$location', 'localStorageService', '$modal', 'configurationFields', function($scope, SettingsService, $state, $rootScope, UtilsService, CommService, UpgradeService, $location, localStorageService, $modal, configurationFields) {
        ...
        $rootScope.$on('setup_initialized', function() {
            ...
            else 
            {
                ...
                CommService.createNewEvent('popup_created', { 'selected_theme': SettingsService.setup().widgets[0].theme });
        ...
        $scope.addNewWidget = function() {
            ...
            if (SettingsService.hasFeature('multiplePopups')) {
                ...
                UtilsService.premiumMessage('You can create only 1 widget with the lite plan. To create more, please upgrade.')
            }
        }

        $scope.changeEnable = function(state, name) {
            var message = 'This Popup will be Disabled'
            var label = 'Disable'
            if (state) {
                message = 'This Popup will be Enabled'
                label = 'Enable'
            }
        ...
        $scope.isEmailTheme = function(index) {
            ...
            if (widgets[index].components.emailSubscription) {
                ...
                if (current_widget_provider == 'Yeloni') {
                    return true
                }
            }
            return false
        ...
        $scope.sendFeedback = function(feedback,closed) {
            ...
            CommService.createNewEvent('submitted_feedback_form', {
        ...
        SettingsService.initialize(function() {
            ...
            CommService.getAllDetails(yetience.product)
                .then(function(all_details) {
                    ...
                    // // console.log('removed extra values')
                    // // console.log($rootScope.components['email-subscriber'].fields[0].templateOptions.options)
                    // // console.log(temp_array)


                    // //adding it to template options - for formly
                    // temp.templateOptions = {
                    //         "label": "Select Premium Email Provider",
                    //         "labelProp": "name",
                    //         "valueProp": "id",
                    //         "options": temp_array
                    //     }
                    ...
                    // // pushing it so that it comes in the formly dialog
                    // $rootScope.components['email-subscriber'].fields.push(temp)
                    //     // console.log('added')
                        //premium email provider end
...
angular.module('yetienceApp')
.controller('GoalsController', ['$rootScope', function($rootScope) {
	var goals = $rootScope.goals
}])
angular.module('yetienceApp')
    ...
    .controller('buildSelectController', ['$scope', 'SettingsService', '$state', '$rootScope', 'CommService', 'UtilsService', 'WidgetUpdate', '$modal', 'UpgradeService', function($scope, SettingsService, $state, $rootScope, CommService, UtilsService, WidgetUpdate, $modal, UpgradeService) {
        ...
        $scope.selectTheme = function(label, theme) {
            ...
            if (label == 'blank-popup' && !SettingsService.hasFeature('customHtml')) {
                ...
                CommService.createNewEvent("blank_popup_selected_free");
                //if the user does not have acces to premium themes, show an alert message
                //alert('For Premium Themes, Please Upgrade your Subscription Plan')
...
angular.module('yetienceApp')
    .controller('buildLaunchController', ['$scope', 'SettingsService', 'CommService', 'UpgradeService', '$stateParams', '$state', function($scope, SettingsService, CommService, UpgradeService, $stateParams, $state) {
        ...
        $state.go('build.launch.statistics')
...
angular.module('yetienceApp')
    .controller('extensionSaleController', ['$scope', 'CommService', 'SettingsService', '$rootScope', 'localStorageService', function($scope, CommService, SettingsService, $rootScope, localStorageService) {
        ...
        $scope.priceForFeature = function(label,duration) {
            ...
            var yearlyPrice = 50;
            var monthlyPriceCombinedFeature = 10;
            ...
            else if(duration == 12)
            {
                ...
                else
                {
                 ...
                 if (label == 'premiumSubscription') {
                    //asking for a yearly price of combined extension
                    return monthlyPriceCombinedFeature;
                }
                else
                {
                    //asking for a yearly price of single extension
...
angular.module('yetienceApp')
    .controller('ModalFeedbackController', ['$scope', 'CommService', '$modalInstance', 'configurationFields', function($scope, CommService, $modalInstance, configurationFields) {
        ...
        $scope.configFields = angular.copy(configurationFields)
            /*
        $scope.$watch('feedback.working',
            function(feedback) {

                console.log('feedback.working is', feedback)
                if (feedback) {
                    switch (feedback) {
                        case 'yes':
                            CommService.createNewEvent('popup_feedback_success')
                            break;
                        case 'no':
                            CommService.createNewEvent('popup_feedback_fail')
                            break;
                        case 'issues':
                            CommService.createNewEvent('popup_feedback_issue')
                            break;
                        default:
                            console.log('invalid feedback')
                    }
                    CommService.updateFeedback(feedback.toUpperCase())
                }
            }
        );
*/
...
angular.module('yetienceApp')
    .directive('componentForm', ['$rootScope', function($rootScope) {
        // Runs during compile
        return {
            controller: ['$scope', '$element', 'CommService', '$modal', '$stateParams', '$rootScope', 'SettingsService','UtilsService', function($scope, $element, CommService, $modal, $stateParams, $rootScope,SettingsService,UtilsService) {
                $scope.cust = {
                    fields: [],
                    values: {}
                }
...
angular.module('yetienceApp')
    .directive('emailConnector', ['$rootScope', function($rootScope) {
        ...
        return {
            ...
            restrict: 'A',
            template: "<button ng-hide='!list_requested || fetched_lists' type='button' class='btn btn-primary'  ng-click='openPopup()'>Connect with {{provider}} </button>",
            scope: {
                provider: '@',
                emailConfig: '=',
                emailFields: '='
        ...
        function addAdvancedConfigurationOnSetup(setup, website) {
            ...
            if (setup.widgets) {
                ...
                for (var i = 0; i < setup.widgets.length; i++) {
                    ...
                    if (hasFeature('mobileScreens') || hasFeature('specificPages')) {
...
angular.module('yetienceApp')
    .constant('configurationFields', {
        ...
        feedback: [{
            ...
            className: "section-label",
            template: "Thank you for choosing Yeloni. If you have any questions, write to us at admin@yeloni.com",
